...
static void response_cb(struct http_response* rsp,
    ...
    if (ep->truncated) { ep->recv_len = sizeof(ep->recv_buf); } 
    else { ep->recv_len = rsp->data_len; }
...
int http_get_uint32(struct http_endpoint* ep,
                    const char* path,
                    uint32_t* value) {
    int ret = http_get(ep, path);
    if (ret){return ret; }

    if (ep->status < 200 || ep->status >= 300){
        return -EIO;
    }
    if (ep->truncated){return -EMSGSIZE; }

    char *body = http_body(ep);
    if (body == NULL){return -EINVAL; }

    size_t body_len = ep->recv_len - (body - (char*)ep->recv_buf);

    /* Parse first chunk size (hex). */
    errno = 0;
    char *chunk_end;
    unsigned long chunk_size = strtoul(body, &chunk_end, 16);
    if (chunk_end == body){return -EINVAL; }
    if (errno == ERANGE){return -ERANGE; }

    /* Chunk size line must end with CRLF. */
    size_t chunk_header_len = chunk_end - body;
    if (chunk_header_len + 2 > body_len){return -EINVAL; }

    if (chunk_end[0] != '\r' || chunk_end[1] != '\n'){
        return -EINVAL;
    }
    chunk_end += 2;
    body_len -= chunk_header_len + 2;

    /* Ensure the first chunk contains at least one byte. */
    if (chunk_size == 0 || chunk_size > body_len){
        return -EINVAL;
    }

    /* Parse integer from first chunk data. */
    errno = 0;
    char *end;
    unsigned long parsed = strtoul(chunk_end, &end, 10);
    if (end == chunk_end){return -EINVAL; }

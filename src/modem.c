...
void parse_timer(const char* const timer_str, psm_timer_t* const timer) {
    uint8_t encoded = (uint8_t)strtol(timer_str, NULL, 2);

    timer->unit  = (encoded >> 5) & 0x07;
    timer->value = encoded & 0x1F;
}

int32_t tau_to_seconds(const psm_timer_t* const timer) {
    ...
    if (timer->unit == 7){ return -1; }

    return timer->value * multiplier[timer->unit];
}

int32_t active_time_to_seconds(const psm_timer_t* const timer) {
    static const uint32_t multiplier[] = {
        2,      // 2 s
        60,     // 1 min
        360,    // 6 min
    ...
    if (timer->unit == 7) { return -1; }
    if (timer->unit >= 3) { return -2; }

    return timer->value * multiplier[timer->unit];
}

int modem_psm_mode(uint8_t* const enabled_p, psm_timer_t* const tau_p, psm_timer_t* const active_time_p) {
    char tau_str[9];
    char active_str[9];
    ...
    if (ret == 3) {
        parse_timer(tau_str, tau_p);
        parse_timer(active_str, active_time_p);
        return 0;
    }
    ...
    *enabled_p = -1;
    tau_p->unit = 0xFF;
    tau_p->value = 0xFF;
    active_time_p->unit = 0xFF;
    active_time_p->value = 0xFF;
